...
Calculate the desired drift correction amount, just excluding ones larger than: 
```{r}
...
eachCriterionEveryTrial <- eachCriterionCheckedEveryFixatn |>  filter(sttime_rel > trialMinTimeBeforeCuesOff) |>
      ...
      summarise(anyTooFarAway = any(lookedTooFarAway),
                numTooFarAway = sum(lookedTooFarAway))
```
