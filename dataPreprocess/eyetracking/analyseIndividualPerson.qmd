...
eachCriterionEveryTrial <- eachCriterionCheckedEveryFixatn |>  filter(sttime_rel > trialMinTimeCuesOff) |>
...
tooFarEachTrial<- fixatns |> filter(sttime_rel > trialMinTimeCuesOff) |>
...
Finally, produce a report of whether each character in the Starwars universe is allowed to box in the Olympics in each men's weight class.
```{r}
#| echo: false


                        #Featherweight,..... Super Heavyweight
...
#Allowed contains only the calculated columns, not the original.
#dplyr made this choice because this is a new dataframe, with different number of rows.
#But it means we don't have the values that were the inputs to generate these new values
...
#Combine the new columns with the old
starwarsBoxingEligibility<- cbind(longer_starwars[,1:4],allowed)

starwarsBoxingEligibility
```
