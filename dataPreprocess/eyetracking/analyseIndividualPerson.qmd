---
title: "Examine eyetracking data for an individual participant, and work out how to do drift correction"
format: html
editor: visual
---
...
Will implement drift correction, but need to decide on limited interval at beginning of trial to assume gaze is central.
...
fixatns %>% filter(sttime_rel > trialMinTimeBeforeCuesOff) %>% 
  ...
  geom_point() + geom_line(aes(group=trial)) + facet_wrap(.~quartile)
```
## Calculate exclusions before drift correction

This is mainly to check there is no discrepancy with summariseFromEDF.R

```{r}
#| echo: false
centralZoneWidthPix<- 200
centralZoneHeightPix<- 200
...
#Calculate whether each fixation was outside of central area
...
#Calculate whether *any* fixations on each trial was outside of central area
outOfCentral<- outOfCentral |> group_by(trial) |>
      summarise(anyTooFar = any(outOfCentralArea))
mean(outOfCentral$anyTooFar)
...
```{r}
distFromCenterCriteria <- c(20,40,60,100,200)
```
...
message('Ignoring fixations before trialMinTimeBeforeCuesOff=',trialMinTimeBeforeCuesOff,'. Drift correction=',doDriftCorrection,'with intervalToAssumeWasCentral=',intervalToAssumeWasCentral[1],',',intervalToAssumeWasCentral[2]) 
